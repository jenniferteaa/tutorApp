...
`).length}function A(){y.clear(),k=!1,M!==null&&(window.clearTimeout(M),M=null)}async function R(e){const t=Te(),i=T()?.value??"",o=Array.from(y)[0]??1;if(!o){A();return}const l=Date.now();if(K===o&&l-Z<250)return;if(K=o,Z=l,!t){A();return}let s="";if(i&&(s=te(i,o)),!s.trim()&&o>1&&i){const c=te(i,o-1);c.trim()&&(s=c)}let g=t;try{const c=await b.runtime.sendMessage({type:"GET_MONACO_CODE"});c?.ok&&typeof c.code=="string"&&(g=c.code)}catch{}xe(s)&&(q.push([g,s]),we()),A()}function xe(e){const t=e.trim();return t?/[;}]\s*$/.test(t)?!0:t==="else"||t==="if"||t==="while"||/^else\b/.test(t)&&/\{\s*$/.test(t)?!1:t.replace(/[{}();]/g," ").trim().split(/\s+/).filter(Boolean).length>1:!1}function te(e,t){return e.split(`
